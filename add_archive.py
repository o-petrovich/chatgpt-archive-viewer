...
def conv_date(conv):
    ...
    return datetime.fromtimestamp(ts).strftime("%Y-%m-%d") if isinstance(ts,(int,float)) else datetime.now().strftime("%Y-%m-%d")
...
def find_set_from_conversations(conversations_file):
    conversations_file=Path(conversations_file).resolve()
    folder=conversations_file.parent
    prefix=prefix_from_conversations(conversations_file)
    if prefix:
        idx=folder/f"{prefix}_attachments-index.json"
        manifest=folder/f"{prefix}_run-manifest.json"
        volumes=sorted(folder.glob(f"{prefix}_attachments-vol*.zip"))
    else:
        idx=folder/"attachments-index.json"
        manifest=folder/"run-manifest.json"
        volumes=sorted(folder.glob("attachments-vol*.zip"))
...
def save_catalog(catalog):
    catalog.sort(key=lambda x:(x.get("date",""),x.get("title","")),reverse=True)
...
def import_conversations_file(conversations_file):
    fs=find_set_from_conversations(conversations_file)
    missing=validate_set(fs)
    if missing:raise RuntimeError("Не вистачає файлів: "+", ".join(missing))

    data=load_json(fs["conversations"])
    ...
    index=load_json(fs["attachments_index"])
    all_atts=index.get("attachments",{}) or {}
    tmp=ROOT/".import_tmp"
    ...
    try:
        ...
        for conv in conversations:
            conv_id=conv.get("conversation_id") or conv.get("id") or slug(conv.get("title"))
            title=conv.get("title") or "Без назви"
            date=conv_date(conv)
            folder_name=f"{date}_{slug(title)}"
            dest=CHATS/folder_name

            if dest.exists() and (dest/"conversation.json").exists():
                try:
                    old=load_json(dest/"conversation.json")
                    old_id=old.get("conversation_id") if isinstance(old,dict) else None
                except Exception:old_id=None
                if old_id and old_id!=conv_id:
                    folder_name+=f"_{str(conv_id)[:8]}"
                    dest=CHATS/folder_name
            ...
            for pointer,info in all_atts.items():
                if not isinstance(info,dict) or info.get("convId")!=conv_id:continue
                subset[pointer]=info
                rel=info.get("path")
                if rel:
                    src=tmp/rel
            ...
            entry={"id":conv_id,"title":title,"date":date,"path":f"chats/{folder_name}"}
            catalog=[x for x in catalog if x.get("id")!=conv_id]
            catalog.append(entry);added.append(entry)

        save_catalog(catalog)
        return {"added":added,"file_set":fs}
    finally:
...
def main():
    ...
    p.add_argument("conversations_file")
    p.add_argument("--delete-source",action="store_true")
    a=p.parse_args()
    result=import_conversations_file(a.conversations_file)
    print(f"Додано чатів: {len(result['added'])}")
    if a.delete_source:
        print(f"Видалено source-файлів: {len(delete_source_set(result['file_set']))}")
